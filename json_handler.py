    ...
    for item in content['records']['data']:
        ...
        if item['expiryDate']==expiry:
            ...
            if 'CE'in item.keys():
                ...
                strikes.append(f"{item['strikePrice']}CE")

            if 'PE'in item.keys():

                strikes.append(f"{item['strikePrice']}PE")

    return strikes



def get_oi(expiry, contract_strike):

    content = {}

    contract=contract_strike[-2:]
    strike=int(contract_strike[:-2])
    ...
    for item in content['records']['data']:
        ...
        if item['expiryDate']==expiry and item['strikePrice']==strike:

            return item[contract]['openInterest']




#x = get_strikes('31-Mar-2022')
#x = get_oi('31-Mar-2022','17200PE')
#print(x)
